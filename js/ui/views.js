...
export function renderAnalysis(summary, { diagnostic = null, adaptive = null, trend = {}, parentSummary = null } = {}) {
  ...
  const diagnosticPanel = diagnostic ? `<section class="diagnostic-panel"><div class="section-heading"><div><span class="eyebrow">PERSONAL DIAGNOSTIC</span><h2>${escapeHtml(diagnostic.student)}・${escapeHtml(diagnostic.source)}診斷</h2></div></div><p class="muted">推薦順序：英文閱讀 → 自然理化 → 數學非選 → 歷史／公民。這是初始底盤；後續會依實際作答動態修訂。</p><div class="diagnostic-list">${diagnosticCards(diagnostic).map((card) => `<article><div><strong>${escapeHtml(card.label)}</strong><span>${escapeHtml(card.result)}</span></div><b>${escapeHtml(card.priority)}</b><p>${escapeHtml(card.recommendation)}</p></article>`).join('')}</div></section>` : '';
  ...
  const trendPanel = Object.keys(trend??{}).length ? `<section class="diagnostic-panel"><div class="section-heading"><div><span class="eyebrow">7 DAY TREND</span><h2>最近 7 天進步曲線</h2></div></div><div class="diagnostic-list">${SUBJECT_ORDER.filter(id=>trend[id]).map(id=>{const t=trend[id],item=SUBJECTS[id];const sign=t.delta>0?'+':'';return `<article><div><strong>${item.icon} ${item.name}</strong><span>${sign}${t.delta}</span></div><b>熟練度 ${t.first} → ${t.latest}</b><p>${t.delta>0?'持續進步':t.delta<0?'近期需要補強':'目前持平'}・${t.points.length} 個紀錄點</p></article>`;}).join('')}</div></section>` : '';
  const parentPanel = parentSummary ? `<section class="diagnostic-panel"><div class="section-heading"><div><span class="eyebrow">PARENT SUMMARY</span><h2>家長摘要</h2></div></div><div class="adaptive-summary"><span>今日作答 <b>${parentSummary.answeredToday}</b></span><span>到期複習 <b>${parentSummary.dueCount}</b></span><span>AI 剩餘 <b>${parentSummary.aiRemaining}</b></span></div><p class="notice">${escapeHtml(parentSummary.nextAction)}</p>${parentSummary.topWeak?.length?`<div class="diagnostic-list">${parentSummary.topWeak.map((skill,index)=>`<article><div><strong>#${index+1} ${escapeHtml(skill.competency)}</strong><span>P${skill.priorityScore}</span></div><b>熟練度 ${skill.mastery}/100</b><p>${escapeHtml(SUBJECTS[skill.subject]?.name??skill.subject)}・${escapeHtml(skill.domain??'')}</p></article>`).join('')}</div>`:''}</section>` : '';
  return `<div class="app-shell"><header class="page-header"><a href="#/">←</a><div><span class="eyebrow">ABILITY SCAN</span><h1>能力分析</h1></div><span></span></header>${subjectRows ? `<section class="analysis-list">${subjectRows}</section>` : '<section class="empty-state"><span>📊</span><h2>還沒有足夠資料</h2><p>完成每科至少 3 題後，這裡會開始顯示強項與弱點。</p><a href="#/">開始第一場冒險</a></section>'}${adaptivePanel}${trendPanel}${parentPanel}${diagnosticPanel}${nav('analysis')}</div>`;
}
...
export function renderProfile({ player, aiUsage={count:0,limit:12}, cycleCount=0, currentCycleStartedOn=null }) {
  ...
  <section class="profile-card"><span class="avatar">🧑‍🚀</span><h2>${escapeHtml(player.name)}</h2><strong>LV.${player.level}</strong><dl><div><dt>累積 EXP</dt><dd>${player.exp}</dd></div><div><dt>金幣</dt><dd>${player.coins}</dd></div><div><dt>總答題</dt><dd>${player.totalAnswered}</dd></div><div><dt>擊敗 Boss</dt><dd>${player.bossesDefeated}</dd></div></dl></section>
  <section class="practice-panel"><span class="eyebrow">AI BUDGET</span><h2>今日 AI 題目額度</h2><p>已使用 ${used} / ${limit} 題。超過上限後會自動改用本地題庫，不會中斷練習。</p></section>
  <section class="practice-panel"><span class="eyebrow">RE-DIAGNOSIS</span><h2>重新建立學習底盤</h2><p>進行 25 題五科平衡診斷，每科 5 題。完成後會用這次結果重建核心能力 Mastery 與 Priority。</p><button class="primary-button" data-action="start-diagnostic">開始 25 題重新診斷</button></section>
