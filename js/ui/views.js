...
export function renderAnalysis(summary, { diagnostic = null, adaptive = null, trend = {}, parentSummary = null, errorReasons = [] } = {}) {
  ...
  const errorPanel = errorReasons.length ? `<section class="diagnostic-panel"><div class="section-heading"><div><span class="eyebrow">ERROR PATTERNS</span><h2>錯因統計</h2></div></div><div class="diagnostic-list">${errorReasons.slice(0,6).map(item=>`<article><div><strong>${escapeHtml(item.label)}</strong><span>${item.count} 次</span></div></article>`).join('')}</div></section>` : '';
  return `<div class="app-shell"><header class="page-header"><a href="#/">←</a><div><span class="eyebrow">ABILITY SCAN</span><h1>能力分析</h1></div><span></span></header>${subjectRows ? `<section class="analysis-list">${subjectRows}</section>` : '<section class="empty-state"><span>📊</span><h2>還沒有足夠資料</h2><p>完成每科至少 3 題後，這裡會開始顯示強項與弱點。</p><a href="#/">開始第一場冒險</a></section>'}${adaptivePanel}${trendPanel}${errorPanel}${parentPanel}${diagnosticPanel}${nav('analysis')}</div>`;
}
