...
export function renderRevenge({ items = [], date = '' }) {
  ...
  return `<div class="app-shell"><header class="page-header"><a href="#/exam-center">←</a><div><span class="eyebrow">SPACED REVIEW</span><h1>錯題與不確定題複習</h1></div></header><p class="notice">今天到期 ${due} 題・共 ${items.length} 題。先自行作答，再看解析；同一天重做不會重複增加熟練度。熟悉的題目也會安排之後再確認。</p>${items.length?`<section class="review-list">${sorted.map(item=>`<article><b>${['🔴','🟠','🟡','🟢'][item.mastery]??'🔴'}</b><div><h3>${escapeHtml(item.topic??item.questionId)}</h3><p>${item.source==='official'?'官方原題':'原創練習'}・熟練度 ${item.mastery}/3・答錯 ${item.wrongCount} 次${item.uncertainCount?`・不確定 ${item.uncertainCount} 次`:''}</p><p>${item.nextReview<=date?'今天到期':'下次複習：'+escapeHtml(item.nextReview)}</p>${reasonSelect(item.questionId,item.reason)}<button class="primary-button" data-action="start-revenge" data-id="${escapeHtml(item.questionId)}">${item.nextReview<=date?'開始到期複習':'提前練習'}</button></div></article>`).join('')}</section>`:'<section class="empty-state"><span>✨</span><h2>目前沒有待複習題目</h2><p>答錯或標記不確定的題目會出現在這裡。</p><a href="#/exam-center">選擇練習</a></section>'}${nav('revenge')}</div>`;
}
